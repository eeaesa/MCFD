...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        if self.apply_sig:
            ...
            self.signatures = pd.read_csv('/mnt/data1/home/zkx/datasets/TCGA/Processed/dataset_csv_sig/signatures.csv')
        else:
    ...
    def get_split_from_df(self, all_splits: dict, split_key: str='train', scaler=None):
        ...
        if len(split) > 0:
            ...
            split = Generic_Split(
                df_slice, metadata=self.metadata, mode=self.mode,
                signatures=self.signatures, data_dir=self.data_dir, label_col=self.label_col,
                patient_dict=self.patient_dict, num_classes=self.num_classes,
                sample=self.sample, num_patches=self.num_patches,
            )
        else:
            split = None
...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    def __init__(self,
        ...
        self._filter_cases_without_wsi()

    def _check_wsi_exists(self, data_dir, slide_ids):
        for slide_id in slide_ids:
            wsi_path = os.path.join(
                data_dir, 'h5_files', f'{slide_id.split(".")[0]}.h5'
            )
            if not os.path.isfile(wsi_path):
                return False
        return True

    def _filter_cases_without_wsi(self):
        valid_indices = []

        for idx in range(len(self.slide_data)):
            case_id = self.slide_data.loc[idx, 'case_id']
            slide_ids = self.patient_dict[case_id]

            if isinstance(self.data_dir, dict):
                source = self.slide_data.loc[idx, 'oncotree_code']
                data_dir = self.data_dir[source]
            else:
                data_dir = self.data_dir

            if self._check_wsi_exists(data_dir, slide_ids):
                valid_indices.append(idx)

        if len(valid_indices) < len(self.slide_data):
            print(
                f"[Warning] Removed {len(self.slide_data) - len(valid_indices)} "
                f"samples due to missing WSI features."
            )

        self.slide_data = self.slide_data.iloc[valid_indices].reset_index(drop=True)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'omic':  # genomic
            genomic_features = torch.tensor(self.genomic_features.iloc[idx])
            return (torch.zeros((1, 1)), genomic_features, label, event_time, c, clinical_data)

        elif self.mode in ["path_and_geno"]: # prosurv
            genomic_features = torch.tensor(self.genomic_features.iloc[idx]) # load gene
            patch_features, mask = self._load_wsi_embs_from_path(data_dir, slide_ids) # load wsi

            return (patch_features, genomic_features, label, event_time, c, clinical_data, mask)

        elif self.mode in ["coattn"]:  # "MCAT" use this format
            patch_features, mask = self._load_wsi_embs_from_path(data_dir, slide_ids)
        ...
        elif self.mode == "pathways":  # "PIBD" and "survpath" use this format
            patch_features, mask = self._load_wsi_embs_from_path(data_dir, slide_ids)
            omic_list = []
            ...
            raise NotImplementedError('Model Type [%s] not implemented.' % self.mode)
    ...
    def _load_wsi_embs_from_path(self, data_dir, slide_ids):
        ...
        for slide_id in slide_ids:
            wsi_path = os.path.join(data_dir, 'h5_files', '{}.h5'.format(slide_id.split('.')[0]))
            wsi_bag = h5py.File(wsi_path, "r")
            features = wsi_bag['features'][:]
