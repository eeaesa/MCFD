...
class MCAT_Surv(nn.Module):
    ...
    def forward(self, **kwargs):
        x_path = kwargs['x_path'] # [N_p, D]
        x_omic = [kwargs['x_omic%d' % i] for i in range(1,7)] # list=6
        # result = torch.cat([tensor.flatten() for tensor in x_omic])

        h_path_bag = self.wsi_net(x_path).unsqueeze(1) ###  # [N_i, 1, d], path embeddings are fed through a FC layer
        ...
        h_omic_bag = torch.stack(h_omic).unsqueeze(1) ### [6, 1, d], omic embeddings are stacked (to be used in co-attention)

        # Coattn
        # query=h_path_coattn=[6, 1, size_dict_WSI[1]], A_coattn=[1, 1, 6, N_i]
        h_path_coattn, A_coattn = self.coattn(h_omic_bag, h_path_bag, h_path_bag)

        ### Path
        h_path_trans = self.path_transformer(h_path_coattn) # [6, 1, size_dict_WSI[1]]
        # A_path=[6, 1], h_path=[6, size_dict_WSI[1]]
        A_path, h_path = self.path_attention_head(h_path_trans.squeeze(1))
        A_path = torch.transpose(A_path, 1, 0) # [6, 1]
        h_path = torch.mm(F.softmax(A_path, dim=1) , h_path) # [1, size_dict_WSI[1]]
        h_path = self.path_rho(h_path).squeeze() # [size_dict_WSI[1], ]
        
        ### Omic
        h_omic_trans = self.omic_transformer(h_omic_bag) # [6, 1, size_dict_WSI[1]]
        # A_omic=[6, 1], h_omic=[6, size_dict_WSI[1]]
        A_omic, h_omic = self.omic_attention_head(h_omic_trans.squeeze(1))
        A_omic = torch.transpose(A_omic, 1, 0) # [6, 1]
        h_omic = torch.mm(F.softmax(A_omic, dim=1) , h_omic) # [1, size_dict_WSI[1]]
        h_omic = self.omic_rho(h_omic).squeeze() # [size_dict_WSI[1], ]
        
        if self.fusion == 'bilinear':
            h = self.mm(h_path.unsqueeze(dim=0), h_omic.unsqueeze(dim=0)).squeeze()
        elif self.fusion == 'concat':
            h = self.mm(torch.cat([h_path, h_omic], axis=0)) # [size_dict_WSI[1], ]
                
        ### Survival Layer
        logits = self.classifier(h).unsqueeze(0) # [1, num_classer]
...
class NLLSurvLoss_my(object):
    def __init__(self, alpha=0.15):
        self.alpha = alpha

    def __call__(self, hazards, S, Y, c, alpha=None):
        if alpha is None:
            return nll_loss_my(hazards, S, Y, c, alpha=self.alpha)
        else:
            return nll_loss_my(hazards, S, Y, c, alpha=alpha)
...
def nll_loss_my(hazards, S, Y, c, alpha=0.4, eps=1e-7):
    batch_size = len(Y)
...
if __name__ == "__main__":
    ...
    proj_dim = 256
    N_patch = 4096
    x_g = torch.randn(gene_in_dim).cuda()
    x_p = torch.randn(N_patch, wsi_in_dim).cuda()
    label = torch.tensor([1], dtype=torch.long).cuda()
    censor = torch.tensor([1], dtype=torch.long).cuda()
    input_args = {
        "x_path": x_p,
        "x_omic6": torch.randn(100).cuda(),
        "x_omic1": torch.randn(100).cuda(),
        "x_omic2": torch.randn(100).cuda(),
        "x_omic3": torch.randn(100).cuda(),
        "x_omic4": torch.randn(100).cuda(),
        "x_omic5": torch.randn(100).cuda(),
        "training": True,
    }

    model = MCAT_Surv(
        omic_sizes=[100, 100, 100, 100, 100, 100],
        n_classes=4,
    ).to(device)
    ...
    # thop 不直接支持 **kwargs，需要一个 Wrapper 将字典作为单个输入传入
    ...
    wrapper = ThopWrapper(model)
    # 必须把 inputs 包装成 tuple，这里 input_args 是唯一的参数
    macs, _ = profile(wrapper, inputs=(input_args,), verbose=False)
    print(f"Total FLOPs (MACs): {macs / 1e9:.2f} G")
    del wrapper  # 清理

    # ------------------------------------------------------
    # 3. 性能测试设置 (Warm-up & Loop)
    # ------------------------------------------------------
    repeat = 50  # 重复次数，取平均值
    warmup = 10  # 预热次数
    ...
    # ================= 测量推理时间 (Inference Time) =================
    model.eval()
    input_args["training"] = False  # 确保切换到推理模式逻辑

    # 预热 GPU
    print(">>> 正在预热 (Inference)...")
    with torch.no_grad():
    ...
    print(f"Inference Time: {avg_inf_time:.4f} ms")

    # ================= 测量训练时间 (Training Time) =================
    # 包括：前向传播 + Loss计算 + 反向传播
    model.train()
    input_args["training"] = True

    # 这里的 Loss 计算逻辑需要与你的实际逻辑一致
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)  # 需要优化器来清空梯度
    loss_fn = NLLSurvLoss_my(alpha=0.0)
    print(">>> 正在预热 (Training)...")
    for _ in range(warmup):
        optimizer.zero_grad()
        hazards, S, Y_hat, A  = model(**input_args)
    ...
    for _ in range(repeat):
        ...
        train_timings.append(curr_time)

    avg_train_time = np.mean(train_timings)
    print(f"Training Time (Forward+Backward): {avg_train_time:.4f} ms")

    print("-" * 30)
    print(f"SUMMARY:")
    print(f"Params: {total_params / 1e6:.2f} M")
    print(f"FLOPs : {macs / 1e9:.2f} G")
    print(f"Train : {avg_train_time:.2f} ms")
    print(f"Infer : {avg_inf_time:.2f} ms")
    print("-" * 30)